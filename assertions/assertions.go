...
func getTimeFromString(in interface{}) (time.Time, error) {
	...
	if err != nil {
		return time.Time{}, fmt.Errorf("invalid date provided: %q", in)
	}
	...
	if innerErr != nil {
		return time.Time{}, fmt.Errorf("invalid date provided with %q not in RFC3339 format or humanize format", in)
	}
